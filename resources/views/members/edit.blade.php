        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <select id="countryId" name="country" class="select2 countries form-control">
                        ...
                        @foreach($countries as $key => $value)
                            <option {{ $value->country_id == $member->country ? 'selected':''}} value="{{ $value->country_id }}">{{ $value->country_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="City" name="city" class="select2 cities form-control">
                        ...
                        @foreach($countries[($member->country)-1]->cities as $key => $value)
                            <option {{ $value->city_id == $member->city ? 'selected':'' }} value="{{ $value->city_id }}">{{ $value->city_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="address" id="Address" placeholder="Enter Address">{{ (old('address')) ? old('address') : $member->address }}</textarea>
                </div>
            </div>
        </div>
