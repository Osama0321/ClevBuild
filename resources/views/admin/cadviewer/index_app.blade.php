	...
	function cvjs_OnLoadEnd(){
			...
			*/
			...
			if (svgElement && svgElement.querySelector('g')) {
				...
				const apiUrl = "{{route('getTaskByFilters')}}" +  `?floor_id=${floor_id}`;
...
function getTaskbyName(task_name, floor_id, task_type, callback) {
	...
	if (taskId == "") {
			...
			showCustomAlert('No tasks selected.'); 
			return;
		}
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					...
					showCustomAlert('Tasks updated successfully.'); // Call this instead of alert()
					 // showPipeinfo();
					 const apiUrl = "{{route('getTaskByFilters')}}" +  `?floor_id=${floor_id}`;
					 if(taskType == 'pipe'){
						// updatePipesFromAPI();
					...
					showCustomAlert('Failed to update tasks.'); 
				}
			},
			error: function () {
				// alert("An error occurred.");
...
function showCustomAlert(message) {
    $('body').append(`
        <div id="customAlert" style="position:fixed; top:50%; left:50%; transform:translate(-50%, -50%);
        background:#333; color:#fff; padding:15px; border-radius:5px; z-index:1000;">
            ${message}
        </div>
    `);
    setTimeout(() => { $('#customAlert').fadeOut(500, function() { $(this).remove(); }); }, 2000);
}
...
function getCookie(name) {
    ...
    for (let i = 0; i < cookies.length; i++) {
        ...
        if (cookie[0] === name) {
            return decodeURIComponent(cookie[1]); // Decode in case of encoded values
        }
    }
    return null; // Return null if the cookie doesn't exist
}

const sessionValue = getCookie('clevbuild_session');

console.log('clevbuild_session:', sessionValue);
...
<div class="container-fluid mt-4">
	...
	<div class="inlineContent">
      ...
      <div class="mobile-modal-popup" style="display:none">
  ...
  document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".card-header button.close").forEach(function (button) {
        button.addEventListener("click", function () {
            document.querySelector(".mobile-modal-popup").style.display = "none";
        });
    });
});
...
function renderPipeTasks(response) {
    const tasks = response.tasks;

	const toggleLayerColorsCheckbox = document.getElementById("toggle-layer-colors");
	const isChecked = toggleLayerColorsCheckbox.checked;
    // Filter and iterate over all <path> elements with a specific attribute
    ...
    filteredPaths.each(function () {
        ...
        if (taskName) {
            ...
            if (task) {
                ...
                // Get and set the color for the task
                const color = getTaskColor(task.task_status.status_name);

                // Update stroke and fill based on isChecked flag
                if (isChecked) {
                    pathElement.attr('stroke', color);
                    pathElement.attr('fill', color);
                } else {
                    pathElement.removeAttr('stroke');
                    pathElement.removeAttr('fill');
                }
                ...
                // Update parent element's fill if needed
                if (isChecked) {
                ...
                // Make parent and grandparent elements clickable
                pathElement.parent().parent().addClass('cursor-pointer');
                pathElement.parent().parent().css('cursor', 'pointer');
                ...
                // Attach click event listener to show task details
                pathElement.off('click').on('click', function () {
                    jQuery(".cvjs_wait_looper").fadeIn(50);

                    // Reset stroke-width for all pipes to 2 using CSS
                    jQuery('path.pipe, circle.upright-circle, polygon.sidewall-triangle, circle.large-circle.sprinkler')
                        .css('stroke-width', '2');

                    // Highlight clicked pipe
					jQuery(this).css('stroke-width', '4');		

                    // Display task details
                    displayPipeDetail(response, task);
					document.querySelector(".mobile-modal-popup").style.display = "block";
...
function populateDropdown(task_status_id,member_id, statuses, members,task_type) {
  ...
  // if (task_type === "head") {
    // // Disable and make the dropdown readonly
    // statusDropdown.disabled = true;
  // } else {
    // // Enable dropdown for other task types
