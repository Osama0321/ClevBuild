@extends('layouts.admin.app', ['title' => 'All Followers'])
                        ...
                        <div class="row">
                            ...
                            @can('addFollowers')
                                <div class="col-md-3 {{ Auth::user()->user_type == 1 ? 'offset-2' : 'offset-9' }}">               
                                    <div class="form-group">
                                        <a href="{{route('followers.add')}}" class="btn btn-block btn-primary">Add New Follower</a>
                                    </div>
                                </div> 
                            @endcan       
                        </div>
                        <table id="followers" class="table table-bordered table-striped">
                            <thead>
                                <tr>
                                    <th>First Name</th>
                                    <th>Last Name</th>
...
<script src="{{ asset('Admin/plugins/datatables-buttons/js/buttons.colVis.min.js') }}"></script>
	...
	<script type="text/javascript">
		$(document).ready( function () {
		    ...
		    var table = $('#followers').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('followers.get') }}",
                    type: 'GET',
                    data: function(d) {
            ...
            $(document).off("click",".remove_follower").on("click",".remove_follower",function(e)
            {
                let item = $(this);
                e.preventDefault();
                Swal.fire({
