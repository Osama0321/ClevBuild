        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
              <div class="text-right btn-wrapper">
                <a href="{{route('floors.add',['project_id'=>$project_id])}}" class="btn btn-primary d-inline-flex align-items-center" id="add-floor">Add New Floor</a>
              </div>
          </div>
        </div>
      </div>
          ...
          @forelse($floors as $floor)
            <div class="col-lg-4">
				<div class="bg-white rounded-lg contentBox mb-3">
					<a href="{{ route('cadeditorNew', ['floor_id' => $floor->floor_id]) }}" title="View In Cadviewer">
					...
					<div style="float:right;">
						...
						<div class="mb-1" style="text-align: end;" title="View In Cadviewer">
							<a href="{{ route('cadeditorNew', ['floor_id' => $floor->floor_id]) }}" class="btn btn-primary">View In Cadviewer</a>
						</div>						
					</div>
...
@push('scripts')
  <script>
    let layerTemplates = {!! json_encode($layerTemplates) !!} ?? [];
    let userType = @json(Auth::user()->user_type);
    document.getElementById("add-floor").addEventListener("click", function (e) {
        if (userType !== 1 && layerTemplates.length === 0) {
            e.preventDefault();
            Swal.fire({
                title: "Please add layer template before adding floor.",
                icon: "error",
