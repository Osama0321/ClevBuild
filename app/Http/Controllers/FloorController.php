...
use Auth;
use DataTables;
use App\Models\{Projects,User,City,ProjectStatus,Category,Country,Task,Floor,Statuses,TaskLogs,LayerTemplate};
...
class FloorController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Auth::user()->user_type == 6) {
            $layerTemplates = LayerTemplate::where('user_id',Auth::user()->id)->get();
        ...
        return view('admin.floors.listing',compact('floors','project_id','layerTemplates'));
    }
    ...
    public function getFloors(Request $request)
    {
        ...
        ->addColumn('action', function($row){
            ...
            if(Bouncer::can('viewFloors')){
                $actionBtn .='<a href="' . route('tasks').'?floor_id='.$row->floor_id. '" class="mr-1 btn btn-circle btn-sm btn-info" data-toggle="tooltip" title="View Tasks"><i class="fas fa-eye"></i></a>';
            }
            if(Bouncer::can('viewFloors')){
                $actionBtn .='<a href="' . route('cadeditor').'?floor_id='.$row->floor_id. '" class="btn btn-circle btn-sm btn-info" data-toggle="tooltip" title="View In Cadviewer"><i class="fas fa-eye"></i></a>';
            }
            $actionBtn .= '</div>';
            return $actionBtn;
        })
    ...
    public function create(Request $request)
    {
        ...
        $statuses = Statuses::active()->where('status_type','pipe')->orderBy('status_id','asc')->get();
        ...
        $projects = $projects->where('project_id',$request->project_id)->first();
	...
	**/
    ...
    {
        if($request->template_name && $request->template_name != ''){
            $validator = Validator::make($request->all(), [
                'template_name' => 'required|unique:layer_templates|max:80',
            ...
            if ($map_data) {
                ...
                if($request->template_name && $request->template_name != ''){
                    $layerTemplate = array(
                        ...
                        'template_layers' => json_encode($request['layersData']),
